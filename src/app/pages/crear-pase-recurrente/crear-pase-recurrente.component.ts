...
import {INuevaSolicitudRecurrente} from '../../models/solicitudes/NuevaSolicitudRecurrente';
...
@Component({
  selector: 'app-crear-pase-recurrente',
  templateUrl: './crear-pase-recurrente.component.html',
  styleUrls: ['./crear-pase-recurrente.component.css']
})
export class CrearPaseRecurrenteComponent implements OnInit {
  ...
  Titulo: 'Crear pase recurrente';
  Usuario = new IUsuario()
  NuevaSolicitud = new INuevaSolicitudRecurrente();
  empleadoNombre = '';
  ...
  guardar_solicitud()
  {
    try 
    {
      ...
      this.solicitudesService.crear_solicitud_recurrente(this.NuevaSolicitud)
      .then(()=>
      {
  ...
  datos_nueva_solicitud()
  {
    ...
    this.NuevaSolicitud.HoraSalidaEstimada = this.formatear_fecha(this.horaSalida.hour, this.horaSalida.minute )
    this.NuevaSolicitud.HoraEntradaEstimada = this.formatear_fecha(this.horaEntrada.hour, this.horaEntrada.minute )
    this.NuevaSolicitud.idUsuarioAutoriza = this.idUsuarioAutoriza
    this.NuevaSolicitud.FechaDesde = this.fechaDesde.year + "-" + this.fechaDesde.month + "-" +this.fechaDesde.day
    this.NuevaSolicitud.FechaHasta = this.fechaHasta.year + "-" + this.fechaHasta.month + "-" +this.fechaHasta.day
  ...
  setear_fecha()
  {
    this.fechaDesde = {year : new Date().getFullYear(), month : new Date().getMonth()+1,  day: new Date().getDate()+1};
    this.fechaHasta = {year : new Date().getFullYear(), month : new Date().getMonth()+1,  day: new Date().getDate() + 7};
  }


  cerrar_alerta()
  {
    this.alertasService.cerrar_alerta("alertaNuevaSolicitudRecurrente")
  }


  validar_mostrar_alerta()
  {
    var yaSeMostro = this.alertasService.validar_mostrar_alerta("alertaNuevaSolicitudRecurrente")
